...
The bundled probe derives Windows, System32, and Program Files roots from native operating-system APIs; rejects reparse or current-token-writable trusted components; ignores inherited `PATH`; and pins `PSModulePath` to validated system-only module directories. Only `ERROR_ACCESS_DENIED` and `ERROR_PRIVILEGE_NOT_HELD` count as definitive denial of a requested dangerous right; sharing, transient, and unknown errors reject the path. This protects against unprivileged executable and PowerShell-module shadowing, not privileged replacement inside validated system directories. Windows v0.1 rejects `--output`; redirect or capture stdout in a directory whose ownership you have already established.
